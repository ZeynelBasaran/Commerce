...
function Navbar() {
  const { theme, setTheme, setDrawerClose, drawerClose, basket,addToBasket,removeFromBasket } =
  ...
  return (
    <>
      <nav className="flex justify-between container py-4">
        ...
        <ul className="flex items-center">
          ...
          <li className="cursor-pointer" onClick={drawerFunc}>
            <Badge />
          </li>
        </ul>
      </nav>

      <Drawer anchor="right" open={drawerClose} onClose={() => { drawerFunc() }}>
        {basket?.map((item, idx) => {
          return (
